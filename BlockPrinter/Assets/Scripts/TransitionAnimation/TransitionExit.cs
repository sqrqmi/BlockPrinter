...
namespace BlockPrinter.UserInterface
{
    public class TransitionExit : MonoBehaviour
    {
            ...
            for (int x = 0; x < BG_X; x++)
            {
                for (int y = 0; y < BG_Y; y++)
                {
                    ...
                    while (!IsDone)
                    {
                        ...
                        if (this.Blocks[index].IsDone == false)
                        {
                            StartCoroutine(FadeBlock(this.Blocks[index]));
        ...
        private IEnumerator FadeBlock(AnimationBlock block)
        {
            block.Block.SetAlpha(1f, 0f, this.IntervalTime);
            yield return null;
        }
        ...
        private void GenerateAnimationBlock()
        {
            ...
            for (int y = 0; y < BG_Y; y++)
            {
                for (int x = 0; x < BG_X; x++)
                {
                    ...
                    this.Blocks[index].Block.Highlight();
